...
public sealed class UpdateSkillEntryStyleCommandValidator : AbstractValidator<UpdateSkillEntryStyleCommand>
{
    public UpdateSkillEntryStyleCommandValidator()
    {
        RuleFor(x => x.Id).NotEmpty();

        RuleFor(x => x).Custom((model, context) =>
        {
            if (model.SkillLayout != null)
            {
